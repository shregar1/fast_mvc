...
class ConfigValidator:
    ...
    def validate_database_url(cls, value: str) -> tuple[bool, str]:
        ...
        if not cls.DATABASE_URL_PATTERN.match(value):
            return False, "Invalid database URL format. Expected: postgresql://user:pass@host/db"
    ...
    def validate_jwt_secret(cls, value: str) -> tuple[bool, str]:
        ...
        if len(value) < cls.JWT_SECRET_MIN_LENGTH:
            return False, f"JWT secret too short. Minimum {cls.JWT_SECRET_MIN_LENGTH} characters (got {len(value)})"
        
        # Check for common weak secrets
        weak_secrets = [
            "secret", "password", "123456", "jwt", "token",
            "your-secret-key", "change-me", "default", "test"
        ]
        ...
        if complexity_score < 2:
            return False, f"JWT secret lacks complexity. Use mix of upper, lower, digits, and special characters"
    ...
    def validate_app_env(cls, value: str) -> tuple[bool, str]:
        ...
        valid_envs = ["development", "dev", "staging", "stage", "production", "prod", "test", "testing"]
        
        if value.lower() not in valid_envs:
            return False, f"Invalid environment. Must be one of: {', '.join(valid_envs)}"
